...
class Entry extends REST_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Job_entry_model', 'model');
	}
	...
	{
		$id = $this->post('jobEntryId');
		$where = array('jobEntryId' => $id);
		$data = $this->post();
	...
	{
		$data = $this->post();
		$where = "jobEntryId = {$data['jobEntryId']}";
        //delete the details
        $this->load->model('Job_entry_detail_model', 'entryDetails');
        $result1 = $this->entryDetails->delete($where);
