...
class Job_entry_model extends CI_Model
{
	private $table;
	...
	public function read($id, $filter, $limit=NULL, $offset=NULL, $sort=NULL)
	{
	    ...
	    $this->db->select("{$this->table}.* ,buyerCode, buyerDesc,buyerAddress,buyerContactNo");
        $this->db->from($this->table);
		if($id) {
			$this->db->where("{$this->table}.jobEntryId =", $id);
		}
        //join to buyer to get buyer name
        $this->db->join('buyer','buyer.buyerId = jobentry.buyerId', 'left');
		if( $limit > 0) {
		...
		if(!empty($sort)) {
			...
			$this->db->order_by('jobEntryId', 'desc');
		}
