...
class Job_entry_detail_model extends CI_Model
{
	private $table;
	...
	public function read($id, $filter, $limit=NULL, $offset=NULL, $sort=NULL)
	{
        ...
        $this->db->select("{$this->table}.* , itemsCode, itemsDesc, itemsStyle, unitCode, unitDesc");
        $this->db->from($this->table);

		if($id) {
			$this->db->where("{$this->table}.jedId =", $id);
		}

        //join to buyer to get buyer name
        $this->db->join('items',"items.itemsId = {$this->table}.itemsId", 'left');
        $this->db->join('unit',"unit.unitId = {$this->table}.unitId", 'left');
		if( $limit > 0) {
